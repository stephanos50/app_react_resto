    ...
    return loading ? <Loader /> : error ?  <Message variant='error'>{error}</Message> : <>
        <Row className='p-3'>
            <Col>
                ...
                <h3>Commande :  {format(new Date(order.createAt),'dd-MM-yyyy')} heure: {order.time} </h3>
            </Col>
            <Col>
                ...
                <Col md={8}>
                    <ListGroup variant='flush'>
                        ...
                        {order.payment != null && 
                            <ListGroup.Item>
                                <h2>Livraison</h2>
                                {order.payment !=null &&  ( <Message variant='success'> <h3>Payée</h3> </Message> )}
                                {order.isDelivered ? <Message variant='success'> <h3>Livrée</h3> </Message> : <Message variant='danger'><h3>Non livrée</h3> </Message>}
